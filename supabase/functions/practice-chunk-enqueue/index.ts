 ...
 *   - enqueue_rechunk_v1:        docs without v1 chunks (backfill)
...
serve(async (req) => {
  ...
  try {
    ...
    // ─── Enqueue rechunk v1 backfill (cursor-paginated) ──────
    if (action === "enqueue_rechunk_v1") {
      const PAGE_SIZE = 2000;
      let cursor = "00000000-0000-0000-0000-000000000000";
      let totalEnqueued = 0;
      let pages = 0;

      // Advisory lock to prevent concurrent backfill runs
      const { data: lockAcquired } = await supabase.rpc("try_backfill_lock");
      if (lockAcquired === false) {
        return new Response(JSON.stringify({ error: "Another backfill is already running" }), {
          status: 409,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      try {
        while (true) {
          const rpcName = source === "knowledge_base"
            ? "get_kb_docs_without_v1_chunks"
            : "get_practice_docs_without_v1_chunks";

          const { data: docs, error: rpcErr } = await supabase.rpc(rpcName, {
            p_cursor: cursor,
            p_limit: PAGE_SIZE,
          });

          if (rpcErr) {
            console.error(`[practice-chunk-enqueue] backfill RPC error page=${pages}:`, rpcErr.message);
            break;
          }
          ...
          // Enqueue chunk jobs in batches
          for (let i = 0; i < docIds.length; i += 500) {
            const batch = docIds.slice(i, i + 500).map((id: string) => ({
              document_id: id,
              source_table: source,
              job_type: "chunk",
              status: "pending",
            ...
            if (upsertErr) {
              console.error(`[practice-chunk-enqueue] backfill upsert error:`, upsertErr.message);
            } else {
          ...
          pages++;
          cursor = docIds[docIds.length - 1];
          console.log(`[practice-chunk-enqueue] backfill page=${pages} enqueued=${docIds.length} cursor=${cursor}`);

          // Safety: max 50 pages (100k docs)
          if (pages >= 50) {
            console.warn(`[practice-chunk-enqueue] backfill hit max pages (50), stopping`);
            break;
          }
        }
      } finally {
      ...
      // Fire-and-forget: trigger orchestrator
      const orchUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/practice-pipeline-orchestrator`;
      try {
        const { buildInternalHeaders } = await import("../_shared/edge-security.ts");
        fetch(orchUrl, {
    ...
    if (action === "bulk_enqueue") {
      ...
      console.log(`[practice-chunk-enqueue] bulk_enqueue: ${inserted} jobs for ${source}`);
      ...
      return new Response(JSON.stringify({ enqueued: inserted, source_table: source }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "coverage_stats") {
      const { data, error: rpcErr } = await supabase.rpc("chunk_coverage_stats");
    ...
    if (action === "job_monitor") {
      const { data, error: rpcErr } = await supabase.rpc("pipeline_job_monitor");
      if (rpcErr) throw rpcErr;
      return new Response(JSON.stringify(data), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    return new Response(JSON.stringify({
      error: "Unknown action. Use: diagnostics, enqueue_missing_chunks, enqueue_rechunk_v1, enqueue_missing_embeddings, reset_dead_letters, bulk_enqueue, coverage_stats, job_monitor",
