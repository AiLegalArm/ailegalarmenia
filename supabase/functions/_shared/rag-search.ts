...
export async function searchKB(opts: RAGKBOptions): Promise<RAGResult<KBSearchResult>> {
  ...
  if (merged.length === 0) {
    ...
    if (!error && data) {
      merged = (data as Array<KBSearchResult & { score: number }>)
...
export async function searchPractice(opts: RAGPracticeOptions): Promise<RAGResult<PracticeSearchResult>> {
  ...
  // Case number exact match lookup (parallel with other searches)
  const caseNumberPromise = (async (): Promise<PracticeSearchResult[]> => {
    if (!detectedCaseNumber) return [];
    ...
    return data.map((r: Record<string, unknown>) => ({
      ...
      score: 100, // High priority score for exact case_number matches
  ...
  // deno-lint-ignore no-explicit-any
  const normalizedKeyword = keywordResults.map((r: any) => ({
    ...r,
    case_number: r.case_number_anonymized as string | undefined,
  })) as PracticeSearchResult[];
  ...
  let merged = dedup(caseNumberResults, vectorItems, scoredKeyword);
